...
import sistema.SimpleACS.WalkedWay;

public final class Agente implements Callable<WalkedWay> {

	private final SimpleACS instance;
	...
	public Agente(SimpleACS instance, int start) {
		super();
	...
	private final int getNextProbableNode(int y) {
		if (toVisit > 0) {
			...
			for (int i = 0; i < visited.length; i++) {
				columnSum += Math.pow(instance.readPheromone(y, i), SimpleACS.ALPHA)
						* Math.pow(instance.invertedMatrix[y][i], SimpleACS.BETA);
			}
	...
	private final double calculateProbability(int row, int column, double sum) {
		final double p = Math.pow(instance.readPheromone(column, row), SimpleACS.ALPHA)
				* Math.pow(instance.invertedMatrix[column][row], SimpleACS.BETA);
		return p / sum;
	}
	...
	public final WalkedWay call() throws Exception {
		...
		while ((next = getNextProbableNode(lastNode)) != -1) {
			...
			final double phero = (SimpleACS.Q / (distanceWalked));
